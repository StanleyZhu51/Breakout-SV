...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write val via SPI
	int return_code;
	BYTE writeVal[2] = {reg+2, val};
	return_code = XSpi_Transfer(&SpiInstance, writeVal, NULL, 2);
	//read return code from SPI peripheral (see Xilinx examples) 
	if (return_code != 0) {
		//if return code != 0 print an error
		xil_printf("ERROR on writing!\r\n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	return_code = XSpi_Transfer(&SpiInstance, writeVal, NULL, 1+nbytes);
	if (return_code != 0) {
		//if return code != 0 print an error
		xil_printf("ERROR on writing!\r\n");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
	BYTE readVal[2];
	BYTE writeVal[2] = {reg, 0};
	//write reg via SPI
	//read val via SPI
	return_code = XSpi_Transfer(&SpiInstance, writeVal, readVal, 2);
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if (return_code != 0) {
		xil_printf("ERROR on read!\r\n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	//return val
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
	BYTE writeVal[nbytes+1];
	BYTE readVal[nbytes+1];
	for (BYTE n = 0; n < nbytes+1; n++) {
		writeVal[n] = 0;
	}
	writeVal[0] = reg;
	//write reg via SPI
	//read data[n] from SPI, where n goes from 0 to nbytes-1
	return_code = XSpi_Transfer(&SpiInstance, writeVal, readVal, nbytes+1);
	//read return code from SPI peripheral
	//if return code != 0 print an error
	if (return_code != 0) {
		xil_printf("ERROR on read!\r\n");
